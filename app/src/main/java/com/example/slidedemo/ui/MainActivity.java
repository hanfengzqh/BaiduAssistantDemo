...
public class MainActivity extends FragmentActivity implements PageLoadLayout.OnLoadListener {
        ...
        gv_view.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view,
                ...
                Toast.makeText(mContext, "position = " +
                                categoryList.get(position).getCategory_name(),
    ...
    private void loadData(final EloadType eloadType) {
        ...
        switch (eloadType) {
            case FIRST:
                mAdapter = new RecoListViewAdapter(list, this);
...
//					mainActivity.showSnackbarMessage(EStyle.INFO, "没有更多数据");
     ...
     */
    public void startDownloadAnimation(ImageView iconView,
                                       DownloadAnimationCallback animationCallback) {
        if (iconView != null) {
            ImageView image = new ImageView(this);
            image.setImageDrawable(iconView.getDrawable());

            int[] location = new int[2];
            iconView.getLocationOnScreen(location);

            downloadAnimation(image, location[0], location[1], animationCallback);
        }
    }

    public static abstract class DownloadAnimationCallback {
        public abstract void onAnimationEnd();
    }

    private void downloadAnimation(View view, int x, int y, DownloadAnimationCallback animationCallback) {
        final int[] location = new int[2];
        title_download_manage.getLocationOnScreen(location);
        Log.d("zqh", " location: x2 = " + location[0] + ":y2="
                + location[1]);
        int height = (int) this.getResources().getDimension(
                R.dimen.view_margin_144);
        int width = (int) this.getResources()
                .getDimension(R.dimen.view_margin_144);
        AbsoluteLayout.LayoutParams params = new AbsoluteLayout.LayoutParams(
                width, height, x, y - height / 2);

        mAnimationView.addView(view, params);

        animationText(location[0], location[1], view, animationCallback);

    }

    private void animationText(final int ax, final int ay, final View view,
                               final DownloadAnimationCallback animationCallback) {
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                Log.d("animation", " mAnimationView location: x2 = "
                        + location1[0] + ":y2=" + location1[1]);

                int rX = location1[0] - ax;
                int rY = location1[1] - ay;
                Log.d("animation", " mAnimationViewrX = " + rX);
                Log.d("animation", " mAnimationView rY = " + rY);
                ...
                animationSet.setAnimationListener(new AnimationListener() {
                    ...
                    public void onAnimationEnd(Animation animation) {
                        mHandler.post(new Runnable() {
                            ...
                            @Override
                            public void run() {
                                mAnimationView.removeView(view);
